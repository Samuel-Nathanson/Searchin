...
def filter_search(query):
    ...
    weighted_results_reformat = [{'id':list(x)[0],'relevancy':x[list(x)[0]]} for x in weighted_results]
    ...
    for i,x in enumerate(sorted_weighted_results):
        for j,y in enumerate(result_items):
            #pdb.set_trace()
            if x['id'] == y[0]:
                result_items_temp.append((y[0],y[1],x['relevancy']))
